...
for(const [index,tool] of ['select','v','h','circle','arrow','text','browse','counter','rect'].entries()){
  document.emit('keydown',{key:tool==='rect'?'0':String(index+1),composedPath:()=>[svg]});assert.equal(root.buttons.find(b=>b.dataset.mode===tool).attrs['aria-pressed'],'true');
}
click('mode','browse');
...
pageEvent('pointerdown',editMoveTarget);root.pageGrip.emit('keydown',{key:'ArrowRight'});root.pageEditButton.emit('click');assert.equal(editMoveTarget.getAttribute('contenteditable'),'plaintext-only');
...
pageEvent('pointerdown',movable);root.pageGrip.emit('keydown',{key:'ArrowRight',shiftKey:true});assert.equal(root.pageFloats.children[0].style.left,'101px');
