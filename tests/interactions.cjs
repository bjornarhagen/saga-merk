...
pageEvent('pointerdown',editMoveTarget);root.pageGrip.emit('keydown',{key:'ArrowRight'});root.pageEditButton.emit('click');assert.equal(editMoveTarget.getAttribute('contenteditable'),'plaintext-only');assert.equal(editMoveTarget.style.getPropertyValue('-webkit-user-modify'),'');assert.equal(editMoveTarget.style.getPropertyValue('-moz-user-modify'),'');
