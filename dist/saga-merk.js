(() => {
  ...
  function floatPageElement(el){
    ...
    // Browser editability must follow contenteditable, not a frozen read-only style.
    const moveId=nextId++;
    const styles=nodes.map((node,index)=>{
      ...
      return {node,original:node.getAttribute('style'),marker:node.getAttribute('data-merk-moved-node'),id:`${moveId}-${index}`,pseudo,values:[...computed].filter(name=>!['-webkit-user-modify','-moz-user-modify'].includes(name)).map(name=>[name,computed.getPropertyValue(name)])};
    });
  ...
  for(const input of marginInputs){
    ...
    input.addEventListener('keydown',e=>{
      ...
      const value=Number(input.value),base=input.value.trim()!==''&&Number.isFinite(value)?Math.round(value):16;
    ...
    input.addEventListener('change',()=>{const value=Number(input.value);input.value=String(Number.isFinite(value)&&input.value.trim()!==''?Math.max(0,Math.round(value)):16);updateGrid();showDimensions();});
  }
  ...
  const toolKeys={'1':'select','2':'counter','3':'text','4':'h','5':'v','6':'arrow','7':'rect','8':'circle','9':'browse'};
  function keyboard(e) {
